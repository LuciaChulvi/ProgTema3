...
data1<-ggplot(datos_filtrados,aes(x=date, y = new_deaths_smoothed_per_million, color = location)) + 
  geom_line(size = 0.1)+labs(title='Gráfico de nuevas muertes por millón', x= 'Años', y = 'En millones')+
  geom_line(data = filter(datos_filtrados, location == "Spain"), size = 0.2, col = "black") + 
  theme_classic()+
  geom_hline(yintercept = seq(min(datos_filtrados$new_deaths_smoothed_per_million), max(datos_filtrados$new_deaths_smoothed_per_million), by = 2), linetype = "dashed", color = "gray")+
  scale_y_continuous(breaks =seq(floor(min(datos_filtrados$new_deaths_smoothed_per_million)), ceiling(max(datos_filtrados$new_deaths_smoothed_per_million)), by = 2))+
...
ggplotly(data1) %>% layout(title = list(text = paste0('Gráfico de nuevas muertes en millones')))
```
